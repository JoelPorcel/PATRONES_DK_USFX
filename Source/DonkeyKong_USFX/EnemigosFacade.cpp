...
void AEnemigosFacade::spawn(FString _enemigo, int _cantidad,TMap<int, APlataforma* > _Posicion)
{
	...
	for (int i = 0; i < _cantidad; i++) {
		if (_enemigo.Equals("ave")) {
			Enemigo = GetWorld()->SpawnActor<AEnemigoAve>(AEnemigoAve::StaticClass());
		}
		else if (_enemigo.Equals("dragon")) {
			Enemigo = GetWorld()->SpawnActor<AEnemigos>(AEnemigoDragon::StaticClass());
		}
		else if (_enemigo.Equals("gargola")) {
			Enemigo = GetWorld()->SpawnActor<AEnemigos>(AEnemigoGargola::StaticClass());
...
void AEnemigosFacade::eliminar()
{
	...
	else GEngine->AddOnScreenDebugMessage(-1, 20.f, FColor::Red, FString::Printf(TEXT("No hay enemigos para realizar la tarea")));
}
